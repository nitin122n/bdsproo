// Railway database configuration for BDS PRO
const mysql = require('mysql2/promise');

// Railway connection configuration
const createRailwayConfig = () => {
  return {
...
const getPool = () => {
  if (!pool) {
    ...
    console.log('🚂 Railway connection pool created');
  }
  return pool;
};
...
const testConnection = async () => {
  try {
    ...
    if (error.code === 'ER_ACCESS_DENIED_ERROR') {
      console.log('🔑 Check your Railway credentials in environment variables');
    } else if (error.code === 'ENOTFOUND') {
      console.log('🌐 Check your Railway host URL');
    } else if (error.code === 'ECONNREFUSED') {
      console.log('🔌 Check your Railway port and network connection');
    }
