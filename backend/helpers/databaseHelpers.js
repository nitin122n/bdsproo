...
const createUser = async (userId, name, email, hashedPassword, referralCode = null, referrerId = null) => {
  const query = `
    INSERT INTO users (user_id, name, email, password_hash, referral_code, referrer_id, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())
  `;
  return await executeQuery(query, [userId, name, email, hashedPassword, referralCode, referrerId]);
};
